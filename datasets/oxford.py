...
class OxfordDataset(Dataset):
    ...
    def __init__(self, dataset_path: str, query_filename: str, image_path: str = None,
        ...
        assert os.path.exists(self.query_filepath)
        self.transform = transform
        self.set_transform = set_transform
        
        # '/home/user/vpr/benchmark_datasets/training_queries_baseline.pickle'  [21711]
    ...
    def __getitem__(self, ndx):
        ...
        if self.use_cloud:
            ...
            if self.transform is not None:
                query_pc = self.transform(query_pc) # jitter, removepoint, removeblock for boreas, out[4096,3]
            if self.set_transform is not None and args.dataset in ['oxford','oxfordadafusion']:
                # Apply the same transformation on all dataset elements
                query_pc = self.set_transform(query_pc)
        ...
        if args.dataset in ['oxford','oxfordadafusion']:
            coords, clouds = ME.utils.sparse_quantize(coordinates=query_pc, features=query_pc, quantization_size=args.mink_quantization_size) # boreas quantization_size=1, out[8,3]
            ...
            coords, clouds = ME.utils.sparse_quantize(coordinates=query_pc, features=query_pc, quantization_size=1) # boreas quantization_size=1, out[8,3]
            ...
            # h, w = img.shape[1:]
            # assert w>h
            # scale_factor = h / 2048 # scale_factor = 0.125 for boreas
