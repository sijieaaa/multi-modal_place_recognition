...
class MinkFPN(ResNetBase):
    ...
    def forward(self, x):
        ...
        # output = self.extra_block(x)

        # output = output.dense(min_coordinate=0)


        # x = self.tconv4_2(x)
        # x = self.tconv2_1(x)

        cloud_feat_list = x.decomposed_features
        cloud_coord_list = x.decomposed_coordinates
        cloud_feat_batched = torch.cat(cloud_feat_list)
        cloud_coord_batched = ME.utils.batched_coordinates(cloud_coord_list, dtype=torch.int32, device=x.device)

        
        aa = ME.SparseTensor(features=cloud_feat_batched, coordinates=cloud_coord_batched)

        # a = ME.MinkowskiSPMMAverageFunction
        # a = ME.spmm(x, x)
