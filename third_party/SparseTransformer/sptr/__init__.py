...
class SparseTrODETensor(torch.Tensor):
    def __new__(self, query_feats, query_indices, spatial_shape, batch_size, key_feats=None, value_feats=None, key_indices=None):
        super(SparseTrODETensor, self).__new__()
        """
        Args:
            query_feats: [num_points, num_features] feature tensor
            indices: [num_points, ndim + 1] indice tensor. batch index saved in indices[:, 0]
...
# from .modules import VarLengthMultiheadSAODE